...
if __name__=="__main__":
    ...
    plt.plot(range(0,2*len(p1round),2),p1round)
    plt.plot([compatibility.index(1)] * 2, [0, 1], "k-")
    ...
    plt.plot(range(0,2*len(p2round),2),p2round)
    plt.plot([compatibility.index(1)] * 2, [0, 1], "k-")
    plt.ylabel("Threshold Value")
    plt.xlabel("Round")
    plt.xlim([0,60])
    plt.title("Player 2 Compromise Rate")
    plt.show()
